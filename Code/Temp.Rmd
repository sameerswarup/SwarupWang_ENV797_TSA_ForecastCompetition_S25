...
ARIMA_Four_fit <- auto.arima(ts_temp_train, 
                             seasonal=FALSE, 
                             lambda=0,
                             xreg=fourier(ts_temp_train, 
                                          K=c(2,4))
                             )
...
NN_fit <- nnetar(ts_temp_train,
                 p=1,
                 P=0,
                 xreg=fourier(ts_temp_train, K=c(2,4)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_fit, h=366,xreg=fourier(ts_temp_train, 
                                          K=c(2,4),h=366))
#Plot foresting results
autoplot(NN_for) +
  ylab("Temp") 
...
#Accuracy check
NN_scores <- accuracy(NN_for$mean,ts_temp_test)
NN_scores
```
```{r NNETAR, echo=TRUE, message=FALSE, warning=FALSE}
K=c(2,4)
...
```{r}
NN_fit <- nnetar(temp_ts,
                 p=1,
                 P=0,
                 xreg=fourier(temp_ts, K=c(2,4)))

#NN_for <- forecast(NN_fit, h=365) 
NN_for <- forecast(NN_fit, h=59,xreg=fourier(temp_ts, 
                                          K=c(2,4),h=59))
...
file_path <- "Data/Temp_Forecast.csv"
