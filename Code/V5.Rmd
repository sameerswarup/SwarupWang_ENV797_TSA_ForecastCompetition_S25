...
#NN_fit <- nnetar(ts_act_power_daily_train,p=1,P=1)
NN__w_temp_humid_fit <- nnetar(ts_diff_load_train,
                 p=7,
                 P=1,
                 xreg=fourier(ts_diff_load_train, K=c(2,4)))

#NN_for <- forecast(NN_fit, h=365) 
NN__w_temp_humid_for <- forecast(NN__w_temp_humid_fit, h=365,xreg=fourier(ts_diff_load_train, 
                                          K=c(2,4),h=365))

#Plot foresting results
autoplot(NN__w_temp_humid_for) +
  ylab("Load") 

#Plot model + observed data
autoplot(load_ts) +
  autolayer(NN__w_temp_humid_for, series="Neural Network",PI=FALSE)+
  ylab("Load") 

#Accuracy check
NN_w_temp_humid_scores <- accuracy(NN__w_temp_humid_for$mean,ts_load_test)
